...
- a Go service that can:
  ...
  - bridge terminal-driven clipboard copy requests into the browser's local clipboard when supported, keep persistent tmux sessions in normal text-selection mode, and honor `Cmd-C`/`Ctrl-C` for active terminal text selections without sending an interrupt into the shell
