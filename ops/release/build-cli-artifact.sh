...
TARGET_OS="${FASCINATE_TARGET_OS:-$(go env GOOS)}"
TARGET_ARCH="${FASCINATE_TARGET_ARCH:-$(go env GOARCH)}"
...
build_binary() {
  local artifact_root="$1"
  echo "building CLI binary for ${TARGET_OS}/${TARGET_ARCH}" >&2
  (
    cd "${REPO_ROOT}"
...
main() {
  ...
  if [[ -z "${RELEASE_ID}" ]]; then
    RELEASE_ID="$(build_release_id "cli" "${TARGET_OS}" "${TARGET_ARCH}" "${source_revision}" "${source_dirty}")"
  fi

  bundle_name="fascinate-${RELEASE_ID}"
  WORK_DIR="$(mktemp -d "${OUTPUT_DIR}/build-cli.XXXXXX")"
  artifact_root="${WORK_DIR}/${bundle_name}"
  artifact_path="${OUTPUT_DIR}/${bundle_name}.tar.gz"

  mkdir -p "${artifact_root}"
  copy_artifact_assets "${artifact_root}"
  build_binary "${artifact_root}"
  write_manifest "${artifact_root}" "cli" "${RELEASE_ID}" "${built_at}" "${TARGET_OS}" "${TARGET_ARCH}" "${source_revision}" "${source_dirty}"
  bash "${SCRIPT_DIR}/verify-artifact.sh" --expect-type cli "${artifact_root}" >/dev/null
