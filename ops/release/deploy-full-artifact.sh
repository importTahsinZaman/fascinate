...
usage() {
  cat <<'EOF'
usage: deploy-full-artifact.sh [artifact.tar.gz]
...
build_artifact_path() {
  local output
  output="$(bash "${SCRIPT_DIR}/build-full-artifact.sh")"
  printf '%s\n' "${output}" | tail -n 1
}
...
set -euo pipefail
staging_dir=\$(mktemp -d "${REMOTE_TMP_DIR%/}/fascinate-full.XXXXXX")
cleanup() {
  rm -rf "\${staging_dir}"
  rm -f "${remote_archive}"
}
...
sudo env${remote_env_args} "\${artifact_root}/ops/host/install-control-plane.sh"
EOF
)
