...
func persistentGuestShellCommand(tmuxSession, shellCommand string) string {
	...
	return strings.Join([]string{
		...
		`tmux bind-key -n PageUp if-shell -F '#{pane_in_mode}' 'send-keys -X -N 3 scroll-up' 'copy-mode -eu' >/dev/null 2>&1 || true`,
		`tmux bind-key -n PageDown if-shell -F '#{pane_in_mode}' 'send-keys -X -N 3 scroll-down' >/dev/null 2>&1 || true`,
